...
if st.session_state.authenticated and st.session_state.homepage_displayed:
    ...
    if selected_tab == "Financial Tracking":
        ...
        with st.form("transaction_form"):
            ...
            if submitted:
                if amount <= 0:
                    ...
                    encrypted_amount = encrypt_data(str(amount))  # Convert amount to string before encryption
                    new_transaction = {
                        "date": date,
        ...
        if not df.empty:
            ...
            # Decrypt the amount and convert back to float
            df['amount'] = df['amount'].apply(lambda x: float(decrypt_data(x)))  # Decrypt and convert back to float
    ...
    # Budgeting Tab
        ...
        if not current_budgets.empty:
            ...
            # Decrypt amounts
            expenses['amount'] = expenses['amount'].apply(lambda x: float(decrypt_data(x)))  # Decrypt and convert back to float
    ...
    """
    ### CIA Principles:
    - *Confidentiality*: Uses encryption to protect sensitive data and salted hashes for passwords.
    - *Integrity*: Hashing and limited login attempts prevent unauthorized access and data tampering.
    - *Availability*: SQLAlchemy sessions ensure reliable data access; lockout balances security and usability. Also backup and recovery tab makes sure backup of data is always available.
    """
)
        ...
        st.write("3) Salting of passwords to prevent JohnRipper based Brute Force and Dictionary Attacks")
    ...
    """
    ### Encryption Approach:
    This application uses *Fernet* encryption rather than raw *AES* due to the following advantages:
    
    - *Fernet* provides both encryption and data integrity (via HMAC) in a single package, making it easier and safer to implement securely.
    - *AES* alone requires additional setup for managing keys, initialization vectors (IVs), and integrity checks, which can complicate secure implementation.
    
    *Why Fernet is More Suitable:*
    Fernet handles the complexities of encryption for you, including key generation and tamper detection, making it well-suited for this application where ease of secure implementation is a priority.
    """
)
        ...
        st.write(
            """
            ### Hashing Approach:
            This application uses SHA-256 for password hashing instead of other algorithms 
            like SHA-512 or MD5 for the following reasons:

            - *Security*: SHA-256 provides strong security with a 256-bit hash, making it 
            resistant to brute-force and collision attacks, unlike MD5, which is more vulnerable.
            - *Efficiency*: SHA-256 is faster than SHA-512, making it more efficient while 
            maintaining sufficient security for password hashing.
            - *Salted Hashing*: This implementation uses a unique salt with each password 
            to protect against rainbow table attacks. SHA-256 works well with salted hashing 
            to ensure unique, secure hashes for identical passwords.

            *Why SHA-256 is More Suitable:*
            SHA-256 provides a secure, balanced approach, making it ideal for password hashing 
            in this application. It ensures data integrity and efficiency, balancing both 
            performance and security needs.
            """
        )

        # Timing comparisons for SHA-256, SHA-512, and MD5
        ...
        # Measure the time taken for each hashing function
        sha256_time = timeit.timeit(hash_sha256, number=10000)
        sha512_time = timeit.timeit(hash_sha512, number=10000)
        ...
        st.write(
            ...
            To demonstrate the performance differences, here is a comparison of the time 
            taken for 10,000 iterations of each hashing algorithm:

            - *SHA-256*: {sha256_time:.4f} seconds
            - *SHA-512*: {sha512_time:.4f} seconds
            - *MD5*: {md5_time:.4f} seconds
            """
        )
